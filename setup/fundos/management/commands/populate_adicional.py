...
class Command(BaseCommand):
    help = 'Baixa, extrai e popula a tabela InformeDiario com dados dos meses 05, 06 e 07, evitando duplicações para o mês 05.'

    def handle(self, *args, **kwargs):
        base_url = 'https://dados.cvm.gov.br/dados/FI/DOC/INF_DIARIO/DADOS/inf_diario_fi_{}.zip'
        meses = ['202405', '202406', '202407']  # Maio, Junho, Julho
            ...
            for mes in meses:
                ...
                try:
                    with open(csv_filename, newline='', encoding='latin-1') as csvfile:
                        ...
                        if is_maio:
                            self.stdout.write('Verificando existência dos dados de maio...')
                            objs_existing = set(InformeDiario.objects.filter(DT_COMPTC__year=2024, DT_COMPTC__month=5).values_list('CNPJ_FUNDO', 'DT_COMPTC'))
                        
                        for row in reader:
                            obj_data = {field: row[field] for field in fieldnames}
                            obj = InformeDiario(**obj_data)
                            obj_tuple = (obj.CNPJ_FUNDO, obj.DT_COMPTC)
                            if not is_maio or obj_tuple not in objs_existing:
